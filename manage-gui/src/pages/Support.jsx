...
export default class Support extends React.PureComponent {
  ...
  confirmIncludeInPush = (entityId, type, name) => {
    this.setState({
      confirmationDialogOpen: true,
      confirmationQuestion: I18n.t("support.includeConfirmation", {name: name}),
      confirmationDialogAction: () => {
        this.setState({confirmationDialogOpen: false});
        includeInPush(entityId, type).then(() => this.componentDidMount())
      }
    })
  };
      ...
      <section>
        <table className="search-results">
          ...
          {excludeFromPushServiceProviders.map((entity, index) => <tr
            ...
            <td className="name">
              <Link to={`/metadata/${entity.type}/${entity.id}/metadata`}
            ...
            <td><CheckBox name={entity.id} value={true}
                          onChange={() => this.confirmIncludeInPush(entity.id, entity.type, entity.name)}/>
            </td>
          </tr>)}
          </tbody>
        </table>
